...
// Update the getSalesController to include search functionality
export const getSalesController = async (req, res) => {
    try {
        ...
        let salesData = user.sales;

        // Apply search filter if search parameter exists
        if (search) {
            salesData = salesData.filter(sale => 
                sale.cust_name.toLowerCase().includes(search.toLowerCase()) ||
                sale.cust_email.toLowerCase().includes(search.toLowerCase()) ||
                sale.cust_contact.toLowerCase().includes(search.toLowerCase())
            );
        }

        return res.status(200).json({ status: true, data: salesData });
    } catch (error) {
        console.error(error);
...
export const createNewSaleController = async (req, res) => {
    try {
        ...
        const sale = await Sale.create({cust_name, cust_email, cust_contact, cartItems,userId:req.user.userId});
